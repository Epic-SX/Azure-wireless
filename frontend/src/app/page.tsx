...
const rePr_wireless_API_URL = process.env.NEXT_PUBLIC_rePr_wireless_API_URL || "";
const AUDIO_BUCKET_NAME = process.env.NEXT_PUBLIC_AUDIO_BUCKET_NAME || "";
// Use environment variable for region if available, otherwise fallback to hardcoded value
const AWS_REGION = process.env.NEXT_PUBLIC_AWS_REGION || "ap-northeast-1";

// Log environment variables for debugging (will only show in development)
console.log("API URL:", rePr_wireless_API_URL);
console.log("Audio Bucket:", AUDIO_BUCKET_NAME);
...
const KoenotoApp = () => {
  ...
  const validateAudioUrl = (url: string | undefined): boolean => {
    ...
    // For S3 URLs, they might not have file extensions but are still valid
    if (url.includes('amazonaws.com') || url.includes(AUDIO_BUCKET_NAME)) {
      console.log("S3 URL detected, assuming valid:", url);
      return true;
    }
    
    // Check if URL points to a supported audio format
    const supportedFormats = ['.mp3', '.wav', '.webm', '.ogg', '.m4a', '.aac'];
    const hasValidExtension = supportedFormats.some(format => 
      url.toLowerCase().includes(format)
    );
    
    if (!hasValidExtension) {
      console.warn("URL may not point to a supported audio format:", url);
      // Still return true for URLs without extensions as they might be valid
      return true;
    }
  ...
  const fetchRecordings = async () => {
    try {
      setStatus("一覧取得中...");
      const res = await fetch(`${rePr_wireless_API_URL}/koenoto?user_id=${USER_ID}`);
      if (!res.ok) {
  ...
  const uploadChunks = async (chunks: Blob[]): Promise<string[]> => {
    ...
    for (let i = 0; i < chunks.length; i++) {
      try {
        ...
        // Use backend proxy endpoint to upload to S3
        const uploadResponse = await fetch(`${rePr_wireless_API_URL}/koenoto/upload-audio`, {
          method: "POST",
        ...
        if (!uploadResult.key) {
          throw new Error(`アップロードレスポンスにキーがありません`);
        }
        
        // アップロード成功したら、S3のキーを保存
        uploadedUrls.push(uploadResult.key);
        console.log(`Successfully uploaded chunk ${i} with key ${uploadResult.key}`);
      } catch (error) {
  ...
  const blobToBase64 = (blob: Blob): Promise<string> => {
    return new Promise((resolve, reject) => {
      try {
        setStatus(`データ変換中... (${(blob.size / (1024 * 1024)).toFixed(2)} MB)`);
        ...
        reader.onprogress = (event) => {
          if (event.lengthComputable) {
            ...
            setStatus(`データ変換中... ${percentComplete}%`);
          }
        };
  ...
  const createChunks = (audioBlob: Blob): Blob[] => {
    ...
    // Target chunk size is 1MB (to avoid API Gateway and Lambda payload limits)
    ...
    for (let i = 0; i < chunkCount; i++) {
      ...
      // Create a chunk from the slice of the original blob
      const chunk = audioBlob.slice(start, end, audioBlob.type);
      chunks.push(chunk);
      
      console.log(`Created chunk ${i+1}/${chunkCount}: ${chunk.size / 1024 / 1024} MB`);
    }
    ...
    try {
      ...
      const chunkUrls = chunkKeys.map(
        (key: string) => {
          ...
          return `https://${AUDIO_BUCKET_NAME}.s3.${AWS_REGION}.amazonaws.com/${cleanKey}`;
        }
      );
      
      console.log("Generated audio chunk URLs:", chunkUrls);
      ...
      const response = await fetch(`${rePr_wireless_API_URL}/koenoto/process-audio`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          audioKeys: chunkKeys,
  ...
  /* const createrePr_wirelessItem = async () => {
    try {
      setStatus("サーバーに保存中...");
      const now = new Date();
      ...
      const res = await fetch(`${rePr_wireless_API_URL}/koenoto`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    ...
    try {
      setStatus("削除中...");
      const response = await fetch(`${rePr_wireless_API_URL}/koenoto/${selectedRecording.id}`, {
        method: 'DELETE',
      });
  ...
  const startPolling = (executionArn: string) => {
    const interval = setInterval(async () => {
      try {
        const response = await fetch(`${rePr_wireless_API_URL}/koenoto/process-status?executionArn=${executionArn}`);
  ...
  useEffect(() => {
    ...
    if (audioRef.current && document.body.contains(audioRef.current)) {
      try {
        ...
        if (!audioChunks.length || !audioChunks[currentChunkIndex]) {
          console.warn("No valid audio chunk found at index", currentChunkIndex);
          return;
        }

        const audioUrl = audioChunks[currentChunkIndex];
  ...
  const fetchRecording = async (id: string) => {
    try {
      setStatus("録音データを取得中...");
      const response = await fetch(`${rePr_wireless_API_URL}/koenoto/${id}`);
      ...
      if (data.audioChunks && data.audioChunks.length > 0) {
        ...
        // 完全な音声ファイルがある場合
        console.log("Using complete audio URL:", data.audioUrl);
  ...
  const AudioPlayer = () => {
    ...
    let audioSource = "";
    if (audioChunks.length > 0 && audioChunks[currentChunkIndex]) {
      audioSource = audioChunks[currentChunkIndex];
    } else if (uploadedAudioUrl) {
      audioSource = uploadedAudioUrl;
    } else if (selectedRecording?.audioUrl) {
      audioSource = selectedRecording.audioUrl;
    }
    
    // Validate the audio source
    const isValidSource = validateAudioUrl(audioSource);
    console.log("Audio source for player:", audioSource, isValidSource ? "(valid)" : "(invalid)");
    
    // State to track audio loading errors
    ...
    return (
      <div className="space-y-2 sm:space-y-3">
        ...
        {isValidSource && !audioError ? (
          ...
          <div className="p-3 bg-zinc-100 rounded-md text-zinc-600 text-sm">
            <p>[Invalid audio file]</p>
            <button 
              onClick={() => window.open(audioSource, '_blank')}
  ...
  const saveRecording = async () => {
    try {
      ...
      // Save the recording
      const response = await fetch(`${rePr_wireless_API_URL}/koenoto/save-recording`, {
        method: 'POST',
        headers: {
