...
TABLE_NAME = os.environ.get("TABLE_NAME", "rePr-wirelessRecordings")
AUDIO_BUCKET_NAME = os.environ.get("AUDIO_BUCKET_NAME", "rePr-wireless-stack-rePr-wireless-audio-ap-northeast-1")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if http_method == 'POST':
            ...
            if resource == '/koenoto/presigned-url':
                ...
                try:
                    ...
                    # Check if this is a WAV file that will need conversion
                    will_convert = 'wav' in content_type.lower() or filename.lower().endswith('.wav')
                    ...
                    return {
                        ...
                        'body': json.dumps({
                            ...
                            'willConvertToMp3': will_convert  # Indicate if file will be converted
                        }),
                        'headers': CORS_HEADERS
                    }
                except Exception as e:
            ...
            # Handle direct audio uploads from the frontend
            if resource == '/koenoto/upload-audio':
                try:
                    # Parse the request body
                    body = json.loads(event.get('body', '{}')) if isinstance(event.get('body'), str) else event.get('body', {})
                    filename = body.get('filename')
                    ...
                    try:
                        ...
                        logger.info(f"Successfully uploaded audio file to S3: {filename}")
                        
                        # Return the key for further processing
                        return {
                            'statusCode': 200,
                            'body': json.dumps({
                        ...
                        return {
                            'statusCode': 500,
                            'body': json.dumps({'error': f'Failed to upload audio: {str(e)}'}),
                            'headers': CORS_HEADERS
                        }
                except Exception as e:
        ...
        # Add a new endpoint for converting WAV to MP3 after upload
        if resource == '/koenoto/convert-to-mp3':
            try:
                body = json.loads(event.get('body', '{}'))
                ...
                mp3_key = convert_and_upload_to_mp3(input_key, AUDIO_BUCKET_NAME, content_type)
                
                # Generate the URL for the MP3 file
                mp3_url = f"https://{AUDIO_BUCKET_NAME}.s3.{os.environ.get('AWS_REGION', 'ap-northeast-1')}.amazonaws.com/{mp3_key}"
...
def convert_and_upload_to_mp3(input_key, bucket, content_type):
    """
    Downloads a file from S3, converts it to MP3 if it's a WAV file, and uploads it back to S3.
    Returns the new key for the MP3 file.
    """
    try:
        # Check if the file is already an MP3
        if 'mp3' in content_type.lower() or input_key.lower().endswith('.mp3'):
            logger.info(f"File {input_key} is already an MP3, no conversion needed")
            return input_key

        # Only convert WAV files
        if 'wav' not in content_type.lower() and not input_key.lower().endswith('.wav'):
            logger.info(f"File {input_key} is not a WAV file, no conversion needed")
            return input_key

        # Download the file from S3
        local_input_path = f"/tmp/{uuid.uuid4()}{os.path.splitext(input_key)[1]}"
        local_output_path = f"/tmp/{uuid.uuid4()}.mp3"
        
        logger.info(f"Downloading {input_key} to {local_input_path}")
        s3.download_file(bucket, input_key, local_input_path)
        ...
        cmd = [
            ...
            '-ac', '2',      # Stereo
            '-c:a', 'libmp3lame',
            '-q:a', '2',     # High quality setting
            local_output_path
        ]
        
        logger.info(f"Converting WAV to MP3: {' '.join(cmd)}")
        result = subprocess.run(cmd, capture_output=True, text=True)
        
        if result.returncode != 0:
            logger.error(f"Conversion failed: {result.stderr}")
            return input_key  # Return original key if conversion fails
        
        # Generate new key for the MP3 file
        mp3_key = os.path.splitext(input_key)[0] + '.mp3'
        
        # Upload the MP3 file to S3
        logger.info(f"Uploading converted MP3 file to S3 as {mp3_key}")
        s3.upload_file(
            local_output_path, 
            bucket, 
            mp3_key, 
        ...
        logger.info(f"Successfully converted {input_key} to MP3 and uploaded as {mp3_key}")
        return mp3_key
    except Exception as e:
        logger.error(f"Error converting file to MP3: {e}")
        import traceback
